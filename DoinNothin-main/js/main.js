...
function disableBrowserDefaults() {
    ...
    // allow buttons AND the easter egg word
    if (
        e.target.tagName === 'BUTTON' ||
        e.target.closest('button') ||
        e.target.id === 'settings-easteregg'
    ) {
        return true;
    }
        // 阻止其他元素的鼠标按下（防止拖拽选择）
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const trigger = document.getElementById("settings-easteregg");
    const flashImg = document.getElementById("flash-image");

    console.log("Easter egg loaded:", trigger, flashImg);

    if (!trigger || !flashImg) return;
